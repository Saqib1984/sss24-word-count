...
while (condition) {
  ...
  let answer: { sentence: string } = await inquirer.prompt({
    ...
    message: chalk.yellow.bold(
      "Please enter a sentence you want to count words."
    ),
  });
  ...
  console.log(
    chalk.green.bold(`Your sentence count is ${word_count.length} words`)
  );

  console.log(
    chalk.yellow.bold("\t Thank You For Using Words Count Project.\t")
  );
  ...
  let exit = await inquirer.prompt({
    ...
    type: "confirm",
    message: chalk.bgRed(
      `If You Want To Count Another Words, Please Enter "Yes", For Exit Enter "No"`
    ),
    default: false,
  });
  condition = exit.exit;
